...
class HTMLGenerator {
  ...
  generate() {
    (0, _assert.default)(
      this.env === 'production',
      `HtmlGenerator.generate() should only be used in depot build`,
    );
  ...
  getHtmlPath(path) {
    ...
    const htmlSuffix =
      exportStatic &&
      (0, _lodash.isPlainObject)(exportStatic) &&
  ...
  getMatchedContent(path) {
    ...
    if (config.exportStatic) {
      const branch = (0, _reactRouterConfig.matchRoutes)(
        this.routes,
        path,
      ).filter(r => r.route.path);
      const route = branch.length
  ...
  getDocumentTplPath(route) {
    ...
    if (route.document) {
      ...
      (0, _assert.default)(
        (0, _fs.existsSync)(docPath),
        `document ${route.document} don't exists.`,
      );
      return docPath;
    }
  ...
  getContent(route) {
    ...
    if (this.modifyContext)
      context = this.modifyContext(context, {
        route,
      });
    ...
    let headScripts = [];
    let chunks = ['depot'];
    if (this.modifyChunks)
      chunks = this.modifyChunks(chunks, {
        route,
      });
    chunks = chunks.map(chunk => {
      return (0, _lodash.isPlainObject)(chunk)
        ? chunk
        : {
            name: chunk,
          };
    ...
    if (this.modifyMetas)
      metas = this.modifyMetas(metas, {
        route,
      });
    if (this.modifyLinks)
      links = this.modifyLinks(links, {
        route,
      });
    if (this.modifyScripts)
      scripts = this.modifyScripts(scripts, {
        route,
      });
    if (this.modifyStyles)
      styles = this.modifyStyles(styles, {
        route,
      });
