...
function _default(opts = {}) {
  ...
  const builtInPlugins = [
    ...
    './plugins/commands/cli', // './plugins/commands/inspect',
    // './plugins/commands/test',
    './plugins/commands/help', // './plugins/commands/generate',
    // './plugins/commands/rm',
    ...
    './plugins/global-js',
    './plugins/global-css',
    './plugins/base', // './plugins/mountElementId',
    './plugins/mock',
    './plugins/dot-env',
    './plugins/dot-eslint',
    './plugins/proxy',
...
function pluginToPath(plugins, { cwd }) {
  return (plugins || []).map(p => {
    (0, _assert.default)(
      ...
      `Plugin config should be String or Array, but got ${_chalk.default.red(
        typeof p,
      )}`,
    );
    ...
    let oldPath = path; // console.log('plugins::', plugins)
    // console.log(
    //     resolve.sync(path, {
    //       basedir: cwd,
    //     })
    ...
    try {
      if (path === 'depot-plugin-react') {
        ...
        return [
          _resolve.default.sync('depot-plugin-react', {
   ...
   Please try the following solutions:
     ...
     ${
       path.charAt(0) !== '.' && path.charAt(0) !== '/'
         ? `2. install ${_chalk.default.underline.cyan(oldPath)} via npm/yarn`
         : ''
     }
`.trim(),
      );
...
function diffPlugins(newOption, oldOption, { cwd }) {
