...
export default function(api) {
  ...
  function rm(args) {
    assert(
      !config.routes,
      `depot rm is not supported now when routes is configured.`,
    );
    assert(
      args._ && args._.length === 2,
      `Invalid args, checkout depot help rm for more details.`,
    );
    ...
    switch (type) {
      ...
      default:
        throw new Error(
          `Type ${type} not supported, checkout depot help rm for more details`,
        );
    }
