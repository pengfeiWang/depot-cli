...
function getBabelConfig() {
  var isBrowser = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
  ...
  return {
    presets: [[require.resolve('@babel/preset-env'), _objectSpread({
      ...
      modules: false
    } : {})]].concat(_toConsumableArray(isBrowser ? [require.resolve('@babel/preset-react')] : [])),
...
function transform() {
  ...
  var isBrowser = browserFiles && browserFiles.includes(slash(path).replace("".concat(addLastSlash(slash(root))), ''));
  var babelConfig = getBabelConfig(isBrowser);
  log.transform(chalk[isBrowser ? 'yellow' : 'blue']("".concat(slash(path).replace("".concat(cwd, "/"), ''))));
...
function build(dir) {
  ...
  function createStream(src) {
    ...
    return vfs.src([src, "!".concat(join(srcDir, '**/fixtures/**/*')), "!".concat(join(srcDir, '**/.*-production/**/*')), "!".concat(join(srcDir, '**/*.test.js')), "!".concat(join(srcDir, '**/*.e2e.js'))], {
      allowEmpty: true,
      base: srcDir
    }).pipe(through.obj(function (f, env, cb) {
      if (extname(f.path) === '.js' && !f.path.includes("".concat(sep, "templates").concat(sep))) {
        f.contents = Buffer.from(transform({
          content: f.contents,
          path: f.path,
          pkg: pkg,
          root: join(cwd, dir)
  ...
  function createWebpack() {
    ...
    var cb = function cb(err, st) {
      if (!err) {
        console.log(chalk.green('[webpack build done] '), 'success : ', pkg.name + '.js');
      }
    };
