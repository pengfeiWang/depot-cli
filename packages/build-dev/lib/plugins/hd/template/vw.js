...
function _default(baseFontSize, psdWidth) {
  ...
  var docEl = doc.documentElement; // 为了消除安卓dpr乱标的比例
  ...
  if (isIos) {// iOS下不用做什么
    ...
    docEl.removeChild(div); // 如果1rem的真实px跟html.fontSize不符. 那么就要加一个rate缩放了;
  ...
  metaEl.setAttribute('content', 'width=device-width,user-scalable=no,initial-scale=' + scale + ',maximum-scale=' + scale + ',minimum-scale=' + scale); // width/750*100, 为了统一rem为0.01rem = 1px
