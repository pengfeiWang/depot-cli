...
class FilesGenerator {
  ...
  generateFiles() {
    ...
    let entryContent = (0, _fs.readFileSync)(
    ...
    entryContent = entryContent
      .replace(_constants.PLACEHOLDER_IMPORT, '')
      .replace(_constants.PLACEHOLDER_HISTORY_MODIFIER, '')
      .replace(/<%= libraryName %>/g, libraryName)
      .replace(
        _constants.PLACEHOLDER_RENDER,
...
if (process.env.NODE_ENV === 'production') {
  require('./registerServiceWorker');
    ...
    (0, _fs.writeFileSync)(
      ...
      (0, _fs.readFileSync)(paths.defaultRegisterSWTplPath),
      'utf-8',
    );
  }
  ...
  getRouterJSContent() {
    ...
    (0, _assert.default)(
      (0, _fs.existsSync)(routerTplPath),
    ...
    const routeContentString = tplContent
      .replace(_constants.PLACEHOLDER_IMPORT, '')
      .replace(_constants.PLACEHOLDER_ROUTER_MODIFIER, '')
      .replace(_constants.PLACEHOLDER_ROUTER, routerContent)
      .replace(/<%= libraryName %>/g, libraryName);
  ...
  getRouterContent() {
    ...
    if (loading) {
      loadingOpts = `loading: require('${(0, _winPath.default)(
        (0, _path.join)(paths.cwd, loading),
      )}').default,`;
    }

    let routesContent = Object.keys(routesByPath).map(key => {
      const pageJSFile = (0, _winPath.default)(
        (0, _path.relative)(paths.tmpDirPath, routesByPath[key]),
      );
      ...
      const compilingPath = (0, _winPath.default)(
        (0, _path.join)(__dirname, 'Compiling.js'),
      );
