...
var _default = api => {
  api.registerCommand(
    ...
    args => {
      const pkg = require((0, _path.join)(
        process.env.DEPOT_DIR,
        'package.json',
      ));
      ...
      if (args.verbose) {
        const versions = api.applyPlugins('addVersionInfo', {
          initialValue: [
            ...
            `depot-babel-preset@${
              require('depot-babel-preset/package').version
            }`,
            `depot-test@${require('depot-test/package').version}`,
          ],
        });
...
node: ${process.version}
      ...
      if (
        (0, _fs.existsSync)((0, _path.join)(process.env.DEPOT_DIR, '.local'))
      ) {
