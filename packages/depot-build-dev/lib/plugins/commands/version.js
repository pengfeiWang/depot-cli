  ...
  api.registerCommand('version', {
    ...
    if (args.verbose) {
      const versions = api.applyPlugins('addVersionInfo', {
        initialValue: [`depot@${pkg.version}`, `${process.platform} ${process.arch}`, `node@${process.version}`, `depot-build-dev@${require('../../../package').version}`, `af-webpack@${require('af-webpack/package').version}`, `depot-babel-preset@${require('depot-babel-preset/package').version}`, `depot-test@${require('depot-test/package').version}`]
      });
