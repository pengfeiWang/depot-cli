...
function _default(service = {}) {
  ...
  const entryScript = (0, _path.join)(
    cwd,
    `./${paths.tmpDirPath}/${libraryName}.js`,
  );
  ...
  const libAlias = {
    ...
    'react-router-dom': (0, _path.dirname)(
      require.resolve('react-router-dom/package'),
    ),
  ...
  if ((0, _fs.existsSync)(pkgPath)) {
    ...
    if (dependencies.antd) {
      libAlias.antd = (0, _path.dirname)(
        require.resolve((0, _path.join)(cwd, 'node_modules/antd/package')),
      );
    }

    if (dependencies['antd-mobile']) {
      libAlias['antd-mobile'] = (0, _path.dirname)(
        require.resolve(
          (0, _path.join)(cwd, 'node_modules/antd-mobile/package'),
        ),
      );
    }
  }
