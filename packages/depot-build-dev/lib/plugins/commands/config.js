...
function _default(api) {
  ...
  function configHandler(args) {
    switch (args._[0]) {
      ...
      case 'get':
        (0, _assert.default)(args._[1], `key must supplied, e.g. depot config get routes`);
        get(args._[1]);
        break;

      case 'set':
        (0, _assert.default)(args._[1] && args._[2], `key and value must supplied, e.g. depot config set mountElementId root`);
        set(args._[1], args._[2]);
        break;

      case 'delete':
        (0, _assert.default)(args._[1], `key must supplied, e.g. depot config delete externals`);
        rm(args._[1]);
        break;

      default:
        throw new Error(`unsupported action ${args._[0]} for depot config, try list, get, set and delete`);
    }
  }
