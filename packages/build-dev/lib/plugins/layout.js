...
import Layout from '${winPath((0, _path.relative)(paths.tmpDirPath, layoutPath))}';
...
${IMPORT}
  ...
  api.register('modifyRouterContent', ({
    memo
  }) => {
    if ((0, _fs.existsSync)(layoutPath)) {
      return memo.replace('<Switch>', `<Layout menuData={menuData}>
    <Switch>`).replace('</Switch>', ` </Switch>
  </Layout>`);
    } else {
      return memo;
    }
