...
var _getUserConfigPlugins = _interopRequireDefault(
  require('af-webpack/getUserConfigPlugins'),
);
...
function _default(api) {
  ...
  const reactDir = (0, _depotUtils.compatDirname)(
    'react/package.json',
    cwd,
    (0, _path.dirname)(require.resolve('react/package.json')),
  );
  const reactDOMDir = (0, _depotUtils.compatDirname)(
    'react-dom/package.json',
    cwd,
    (0, _path.dirname)(require.resolve('react-dom/package.json')),
  );
  const reactRouterDir = (0, _depotUtils.compatDirname)(
    'react-router/package.json',
    cwd,
    (0, _path.dirname)(require.resolve('react-router/package.json')),
  );
  const reactRouterDOMDir = (0, _depotUtils.compatDirname)(
    'react-router-dom/package.json',
    cwd,
    (0, _path.dirname)(require.resolve('react-router-dom/package.json')),
  );
  const reactRouterConfigDir = (0, _depotUtils.compatDirname)(
    'react-router-config/package.json',
    cwd,
    (0, _path.dirname)(require.resolve('react-router-config/package.json')),
  );
  api.chainWebpackConfig(webpackConfig => {
    webpackConfig.resolve.alias
      ...
      .set(
        'history',
        (0, _depotUtils.compatDirname)(
          'depot-history/package.json',
          cwd,
          (0, _path.dirname)(require.resolve('depot-history/package.json')),
        ),
      )
      .set('@', paths.absSrcPath)
      .set('@tmp', paths.absTmpDirPath)
      .set('depot/link', (0, _path.join)(process.env.DEPOT_DIR, 'lib/link.js'))
      .set(
        'depot/dynamic',
        (0, _path.join)(process.env.DEPOT_DIR, 'lib/dynamic.js'),
      )
      .set(
        'depot/navlink',
        (0, _path.join)(process.env.DEPOT_DIR, 'lib/navlink.js'),
      )
      .set(
        'depot/redirect',
        (0, _path.join)(process.env.DEPOT_DIR, 'lib/redirect.js'),
      )
      .set(
        'depot/prompt',
        (0, _path.join)(process.env.DEPOT_DIR, 'lib/prompt.js'),
      )
      .set(
        'depot/router',
        (0, _path.join)(process.env.DEPOT_DIR, 'lib/router.js'),
      )
      .set(
        'depot/withRouter',
        (0, _path.join)(process.env.DEPOT_DIR, 'lib/withRouter.js'),
      )
      .set(
        'depot/_renderRoutes',
        (0, _path.join)(process.env.DEPOT_DIR, 'lib/renderRoutes.js'),
      )
      .set(
        'depot/_createHistory',
        (0, _path.join)(process.env.DEPOT_DIR, 'lib/createHistory.js'),
      )
      .set(
        'depot/_runtimePlugin',
        (0, _path.join)(process.env.DEPOT_DIR, 'lib/runtimePlugin.js'),
      );
  });
  api.addVersionInfo([
    `react@${
      require((0, _path.join)(reactDir, 'package.json')).version
    } (${reactDir})`,
    `react-dom@${
      require((0, _path.join)(reactDOMDir, 'package.json')).version
    } (${reactDOMDir})`,
    `react-router@${
      require((0, _path.join)(reactRouterDir, 'package.json')).version
    } (${reactRouterDir})`,
    `react-router-dom@${
      require((0, _path.join)(reactRouterDOMDir, 'package.json')).version
    } (${reactRouterDOMDir})`,
    `react-router-config@${
      require((0, _path.join)(reactRouterConfigDir, 'package.json')).version
      ...
      ? {
          depot: [
            ...(process.env.HMR === 'none'
              ? []
              : [_reactDevUtils.webpackHotDevClientPath]),
