...
function handleFile(paths, absPath, memo, file) {
  ...
  if (stats.isDirectory()) {
    ...
    const absLayoutFile = (0, _depotUtils.findJS)((0, _path.join)(absPagesPath, newDirPath), '_layout');

    if (absLayoutFile) {
      addRoute(memo, {
        path: normalizePath(newDirPath),
        exact: false,
        component: `./${(0, _depotUtils.winPath)((0, _path.relative)(cwd, absLayoutFile))}`,
        routes,
        isParamsRoute
      }, {
    ...
    addRoute(memo, {
      path,
      exact: true,
      component: `./${(0, _depotUtils.winPath)((0, _path.relative)(cwd, absFilePath))}`,
      isParamsRoute
    }, {
