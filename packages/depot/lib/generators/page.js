...
function _default(opts = {}) {
  ...
  if (isDirectory) {
    (0, _assert.default)(
      !directoryExists((0, _path.join)(cwd, 'src', 'page', file)),
  ...
  (0, _assert.default)(
    !fileExists(jsTargetPath),
    `file src/page/${file} exists`,
  );
  (0, _assert.default)(
    !fileExists(cssTargetPath),
    `file src/page/${file} exists`,
  );
  const jsTpl = (0, _fs.readFileSync)(
    (0, _path.join)(__dirname, '../../template/page.js'),
    'utf-8',
  );
  const cssTpl = (0, _fs.readFileSync)(
    (0, _path.join)(__dirname, '../../template/page.css'),
    'utf-8',
  );
  const fileName = (0, _path.basename)(file);
  const componentName = (0, _uppercamelcase.default)(fileName);

  const jsContent = _ejs.default.render(
    jsTpl,
    {
  ...
  const cssContent = _ejs.default.render(
    cssTpl,
    {},
    {
      _with: false,
