...
var _getDirectoryRouteConfig = _interopRequireDefault(
  require('./getDirectoryRouteConfig'),
);
...
function routesConfigExists(root) {
  ...
  try {
    for (
      var _iterator = _constants.ROUTES_CONFIG_FILE[Symbol.iterator](), _step;
  ...
  if ((0, _fs.existsSync)(path)) {
    ...
    files.forEach(file => {
      ...
      if (stats.isFile() && EXT_NAMES.indexOf(ext) > -1) {
        const fullPath = (0, _path.join)(
          dirPath,
          (0, _path.basename)(file, ext),
        );
        ret.push({
          path: (0, _winPath.default)(`/${variablePath(fullPath)}`).replace(
            /\/index$/,
            '/',
          ),
          exact: true,
        ...
        try {
          for (
            var _iterator2 = _constants.ROUTE_FILES[Symbol.iterator](), _step2;
            ...
            if (
              (0, _fs.existsSync)(
                (0, _path.join)(absPagesPath, fullPath, routeFile),
              )
            ) {
              if (
                (0, _fs.existsSync)(
                  (0, _path.join)(absPagesPath, `${fullPath}${DOT_JS}`),
                )
              ) {
                throw new Error(
                  `路由冲突，src/page 目录下同时存在 "${fullPath}${DOT_JS}" 和 "${(0,
                  _path.join)(fullPath, routeFile)}"，两者指向同一路由。`,
                );
              }

              ret.push({
                path: (0, _winPath.default)(
                  `/${variablePath(fullPath)}`,
                ).replace(/\/index$/, '/'),
                exact: true,
                component: `./${(0, _path.relative)(
                  cwd,
                  (0, _path.join)(absPagesPath, fullPath, routeFile),
                )}`,
              });
