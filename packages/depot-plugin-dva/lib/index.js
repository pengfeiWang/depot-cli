  ...
  return routes.reduce((memo, route) => {
    return [
      ...
      ...(route.component && route.component.indexOf('() =>') !== 0
        ? getPageModels((0, _path.join)(paths.cwd, route.component), api)
        : []),
...
function isSrcPath(path, api) {
  const paths = api.paths,
    winPath = api.winPath;
  return (
    (0, _depotUtils.endWithSlash)(winPath(path)) ===
    (0, _depotUtils.endWithSlash)(winPath(paths.absSrcPath))
  );
}
...
function _default(api, opts = {}) {
  ...
  function exclude(models, excludes) {
    return models.filter(model => {
      ...
      try {
        for (
          var _iterator = excludes[Symbol.iterator](), _step;
  ...
  function getGlobalModelContent() {
    return exclude(
      ...
      (0, _depotUtils.optsToArray)(opts.exclude),
    )
      .map(path =>
        `
    app.model({ namespace: '${(0, _path.basename)(
      path,
      (0, _path.extname)(path),
    ...
    api.modifyRouteComponent((memo, args) => {
      ...
      if (opts.dynamicImport.loadingComponent) {
        loadingOpts = `LoadingComponent: require('${winPath(
          (0, _path.join)(
            paths.absSrcPath,
            opts.dynamicImport.loadingComponent,
          ),
        )}').default,`;
      }
...
_dvaDynamic({
  ...
  ${loadingOpts}
      ...
      const models = getPageModels(
        (0, _path.join)(paths.absTmpDirPath, importPath),
        api,
      );
...
models: () => [
  ...
  api.addVersionInfo([
    `dva@${
      require((0, _path.join)(dvaDir, 'package.json')).version
    } (${dvaDir})`,
  ...
  api.addPageWatcher([
    (0, _path.join)(paths.absSrcPath, 'models'),
    (0, _path.join)(paths.absSrcPath, 'plugins'),
    (0, _path.join)(paths.absSrcPath, 'model.js'),
    (0, _path.join)(paths.absSrcPath, 'model.jsx'),
    (0, _path.join)(paths.absSrcPath, 'model.ts'),
    (0, _path.join)(paths.absSrcPath, 'model.tsx'),
    (0, _path.join)(paths.absSrcPath, 'dva.js'),
    (0, _path.join)(paths.absSrcPath, 'dva.jsx'),
    (0, _path.join)(paths.absSrcPath, 'dva.ts'),
    (0, _path.join)(paths.absSrcPath, 'dva.tsx'),
  ]);
