...
export default function () {
  ...
  var depNames = pullAll(uniq(Object.keys(pkg.dependencies || {}).concat(include || [])), exclude).filter(function (dep) {
  ...
  var files = uniq([].concat(_toConsumableArray(depNames), ['depot/link', 'depot/dynamic', 'depot/navlink', 'depot/redirect', 'depot/router', 'depot/withRouter', 'depot/_renderRoutes', 'depot/_createHistory', 'react', 'react-dom', 'react-router-dom'])).sort(function (a, b) {
    return a > b ? 1 : -1;
  });
  ...
  var afWebpackConfig = require(_resolveDeps('af-webpack/getConfig')).default(afWebpackOpts);
  ...
  var webpackConfig = _objectSpread({}, afWebpackConfig, {
    ...
    plugins: [].concat(_toConsumableArray(afWebpackConfig.plugins), _toConsumableArray(api.webpackConfig.plugins.filter(function (plugin) {
