...
\r\n${e.toString()}`;
    ...
    if (typeof config.directoryConfigRoute === 'string') {
      const srcPath = /src/.test(config.directoryConfigRoute)
        ? config.directoryConfigRoute
        : `./src/${config.directoryConfigRoute}`;
  ...
  watchConfigs(handler) {
    return this.watch('CONFIG_FILES', _constants.CONFIG_FILES).on(
      'all',
      handler,
    );
  }
